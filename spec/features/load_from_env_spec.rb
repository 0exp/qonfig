...
describe 'Load from ENV' do
  specify 'defines config object by ENV data' do
    ...
    expect(config[:all_in][:QONFIG_SPEC_ARRAY_VALUE_V1]).to         eq([true, true, true, true, false, false, false, false])
    expect(config[:all_in][:QONFIG_SPEC_ARRAY_VALUE_V2]).to         eq([1, 2, 3.4, 5.6, 0.0])
    expect(config[:all_in][:QONFIG_SPEC_ARRAY_VALUE_V3]).to         eq(%w[Qonfig Settings DSL])
    expect(config[:all_in][:QONFIG_SPEC_QUOTED_STRING_VALUE_V1]).to eq('test, please')
    expect(config[:all_in][:QONFIG_SPEC_QUOTED_STRING_VALUE_V2]).to eq('please, test')
