...
module Qonfig::Compacted::Constructor
  ...
  class << self
    ...
    # @api private
    # @since 0.21.0
    def construct(compacted_config, initial_data_set = NO_NITIAL_DATA_SET)
      prevent_incompatible_attributes!(compacted_config, initial_data_set)

      if initial_data_set
        construct_instance_from_data_set(compacted_config, initial_data_set)
      else
        construct_isntance_from_commands(compacted_config)
      end
    end
    ...
    # @api private
    # @since 0.21.0
    def construct_instance_from_data_set(compacted_config, initial_data_set)
    ...
    # @api private
    # @since 0.21.0
    def construct_isntance_from_commands(compacted_config)
